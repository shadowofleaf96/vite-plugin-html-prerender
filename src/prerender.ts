...
import { HtmlPrerenderOptions, RenderedRoute } from "./types.js";
...
const emitRendered = async (options: HtmlPrerenderOptions): Promise<void> => {
  ...
  await server
    ...
    .then(async (renderedRoutes) => {
      console.log(
        "[vite-plugin-html-prerender] Saving pre-rendered routes to output..."
      );
    ...
    .then(async () => {
      ...
      console.log(
        "[vite-plugin-html-prerender] Pre-rendering routes completed."
      );
    })
    .catch(async (e) => {
      await renderer.destroy();
      await server.destroy();
      console.error(
        "[vite-plugin-html-prerender] Failed to prerender routes:",
        e
      );
    });
};
